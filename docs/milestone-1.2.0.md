...
Next session entry point: obtain commit/add authority for M5 phase 10 step 12, then commit the remote-tag checkpoint on `master`.
...
| 10 | 12 | Re-read `origin` refs `refs/tags/1.2.0` and `refs/tags/1.2.0^{}`, record their tag and peeled object IDs, and create a canonical checkpoint commit. | Separate canonical-update and commit/add authority | Remote tag object and peeled release-merge evidence are committed before release publication. | Ready for commit; the remote annotated tag object is `f9184bb0dc36ddddf504c569b9518f97c62543c7`, its message is `con 1.2.0`, and it peels to release merge `d1955986bf7758b653ace6dee1a094ba96263c9c`. |
