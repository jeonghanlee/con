...
Next session entry point: obtain commit/add authority for M5 phase 10 step 14, then commit the GitHub-release checkpoint on `master`.
...
| 10 | 14 | Re-read the GitHub release, record its URL and target, and create a canonical checkpoint commit. | Separate canonical-update and commit/add authority | Release-object evidence is committed before the next dependent action. | Ready for commit; the observed release URL is `https://github.com/jeonghanlee/con/releases/tag/1.2.0`, and its tag, title, status, target, and notes match the reviewed release inputs. |
